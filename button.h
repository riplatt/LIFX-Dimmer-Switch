 ...
 */
  ...
  class button
  {
    ...
    private:
      ...
      unsigned long _debounce = 20;              // ms _debounce period to prevent flickering when pressing or releasing the button
      unsigned long _DCgap = 250;                // max ms between clicks for a double click event
      unsigned long _holdTime = 1500;            // ms hold period: how long to wait for press+hold event
      unsigned long _longHoldTime = 5000;         // ms long hold period: how long to wait for press+hold event
