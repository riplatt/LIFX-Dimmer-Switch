...
void lifx::addLight(uint8_t mac[6], uint32_t port)
{
    ...
    {
        Log.info("lifx::addLight - Looking for MAC: 0x %02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    ...
    Log.info("lifx::addLight - Already in Vector: %s", (found ? "true" : "false"));
    if (found == false)
    {
        Log.info("lifx::addLight - Before: Size of Vector: %d, Bytes: %d", Lights.size(), (sizeof(Lights[0]) * Lights.size()));
        light _light = light();
    ...
    Log.info("lifx::addLight - After: Size of Vector: %d, Bytes: %d", Lights.size(), (sizeof(Lights[0]) * Lights.size()));
}
/*
...
void lifx::removeLight(uint8_t mac[6])
{
    ...
    Log.info("lifx::removeLight - Removing MAC: 0x %02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    uint8_t _index;
    bool found = false;
    ...
    Log.info("lifx::removeLight - Size of Vector: %d", Lights.size());
}
...
{
    Log.info("lifx::cycleColour - Step: %0.4f", step);
    HSBK _hsbk;
    float _hue;
    float _kelvin;
    for (auto &Light : Lights)
    {
        _hsbk = Light.getHSBK();
        Log.info("lifx::cycleColour - Colour Mode: %s", Light.getColorMode() ? "Colour" : "Temperature");
...
{
    Log.info("lifx::dimLights - Step: %0.4f", step);
    HSBK _hsbk;
    float _brightness;
    float _step2;
    for (auto &Light : Lights)
    {
        ...
        Log.info("lifx::dimLights - Step2: %0.4f", _step2);
...
void lifx::msgIn(byte packetBuffer[128])
{
    ...
    */
    ...
    Log.trace("lifx::msgIn --- Size: %d", lifxSize);
    Log.trace("lifx::msgIn --- Origin: %d", lifxOrigin);
    Log.trace("lifx::msgIn --- Tagged: %s", lifxTagged ? "true" : "false");
    Log.trace("lifx::msgIn --- Addressable: %s", lifxAddressable ? "true" : "false");
    Log.trace("lifx::msgIn --- Protocol: %d", lifxProtocol);
    Log.trace("lifx::msgIn --- Source: %d", lifxSource);
    Log.trace("lifx::msgIn -- Frame Address:");
    Log.trace("lifx::msgIn --- Target: 0x %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X", lifxTarget[0], lifxTarget[1], lifxTarget[2], lifxTarget[3], lifxTarget[4], lifxTarget[5], lifxTarget[6], lifxTarget[7]);
    Log.trace("lifx::msgIn --- ReservedA: 0x %02X %02X %02X %02X %02X %02X", lifxReservedA[0], lifxReservedA[1], lifxReservedA[2], lifxReservedA[3], lifxReservedA[4], lifxReservedA[5]);
    Log.trace("lifx::msgIn --- ReservedB: %d", lifxReservedB);
    Log.trace("lifx::msgIn --- AckRequired: %s", lifxAckRequired ? "true" : "false");
    Log.trace("lifx::msgIn --- ResRequired: %s", lifxResRequired ? "true" : "false");
    Log.trace("lifx::msgIn --- Sequence: %d", lifxSequence);
    Log.trace("lifx::msgIn -- Protocol Header:");
    Log.trace("lifx::msgIn --- ReservedA: %d", lifxReservedC);
    Log.trace("lifx::msgIn --- PacketType: %d", lifxPacketType);
    ...
    case _deviceStateService:
    {
        ...
        Log.trace("lifx::msgIn -- Service: %d", _service);
        Log.trace("lifx::msgIn -- Port: %d", _port);
    ...
    case _deviceStatePower:
    {
        ...
        Log.trace("lifx::msgIn -- Level: %d", _level);
    ...
    case _deviceStateWifiInfo:
    {
        ...
        Log.trace("lifx::msgIn -- Signal: %d", _signal);
        Log.trace("lifx::msgIn -- Tx: %d", _tx);
        Log.trace("lifx::msgIn -- Rx: %d", _rx);
        Log.trace("lifx::msgIn -- Reserved: %d", _reserved);
    ...
    case _deviceEchoResponse:
    {
        ...
        Log.trace("lifx::msgIn -- Payload: %ll", _payload);
