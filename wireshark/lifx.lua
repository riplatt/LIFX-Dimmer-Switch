...
function setPower(buffer, pinfo, tree)
	tree:add_le(F.level		, buffer(0, 2))
	tree:add_le(F.duration	, buffer(2, 4))
end
...
F.reserved         = ProtoField.bytes("lifx.reserved"        , "Reserved"             , base.NONE)
...
F.unknown          = ProtoField.bytes("lifx.unknown"         , "Unknown"              , base.NONE)
...
F.ip4Address       = ProtoField.bytes("lifx.ip4Address"      , "IP4 address"          , base.NONE)
F.ip6Address       = ProtoField.bytes("lifx.ip6Address"      , "IP6 address"          , base.NONE)
F.ssid             = ProtoField.bytes("lifx.ssid"            , "SSID (UTF8)"          , base.NONE)
F.password         = ProtoField.bytes("lifx.password"        , "Password (UTF8)"      , base.NONE)
F.securityProtocol = ProtoField.bytes("lifx.securityProtocol", "Security protocol"    , base.NONE , securityProtocolStrings[1])
...
F.month            = ProtoField.bytes("lifx.month"           , "Month"                , base.NONE)
F.year             = ProtoField.uint8("lifx.year"            , "Year"                 , base.DEC)
F.label            = ProtoField.bytes("lifx.label"           , "Label"                , base.NONE)
...
F.level			   = ProtoField.uint16("life.level"			 , "Level"				  , base.HEX)
F.duration		   = ProtoField.uint32("lifx.duration"       , "Duration"             , base.DEC)
